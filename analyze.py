...
from utils import xls_to_csv, merge_cost_type, export_to_csv
...
def analyze(main_data_folder: str = "./data", start_date: datetime = now_time, end_date: datetime = next_week):
    try:
                ...
                for index, row in df.iterrows():
                    ...
                    try:
                        ...
                        if start_date <= valid_date <= end_date:
                            ...
                            print(f"Row {index} starts with a valid date: {valid_date}")
                            print(f"Row {index}: {row.to_dict()}")  # Process row here
                    except Exception as e:
                        # If not a date, skip
                        continue
                merge_result = merge_cost_type(result_data)
                export_to_csv(merge_result, file)
                print(merge_result)
            except Exception as e:
